...
- After `gtme-list`, before `gtme-score`. Input: `runs/<slug>/07-list/tam.jsonl` + the ICP's `icp.scoring.boosts[].signal` (formerly watch_signals). Output: `runs/<slug>/08-signals/signals.jsonl` (+ the standard folder companions `provenance.md` and `decisions.md`)
...
*(Sourcing: `02-research/14-practitioner-signals-enrichment.md`.)*
...
`gtme-score` reads `08-signals/signals.jsonl` + `07-list/tam.jsonl` → fit × signal × recency → tier 1/2/3 routing.
