...
- Input: scored account + contact (`score/scored.jsonl`, `score/scored_contacts.jsonl`). Output: `runs/<slug>/research/research.jsonl` (+ the standard folder companions `provenance.md` and `decisions.md`)
...
- One row **per account** in `research/research.jsonl` (like `score/scored.jsonl`), not one file per account. `contact` is the resolved name from `enrich/prospects.jsonl`; if only a role is known, put the role and add `"needs_contact_resolution": true`.
