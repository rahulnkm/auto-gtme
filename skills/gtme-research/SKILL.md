...
- Input: scored account + contact (`09-score/scored.jsonl`, `09-score/scored_contacts.jsonl`). Output: `runs/<slug>/02-research/research.jsonl` (+ the standard folder companions `provenance.md` and `decisions.md`)
...
- One row **per account** in `02-research/research.jsonl` (like `09-score/scored.jsonl`), not one file per account. `contact` is the resolved name from `08-enrich/prospects.jsonl`; if only a role is known, put the role and add `"needs_contact_resolution": true`.
