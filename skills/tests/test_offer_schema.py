...
RUN = Path(__file__).resolve().parent.parent.parent / "runs" / "mousecat" / "offer" / "offer.json"
...
def test_the_capacity_number_is_checked_across_the_file():
    ...
    with tempfile.TemporaryDirectory() as d:
        os.makedirs(os.path.join(d, "offer"))
        bad = offer(scarcity_facts=["in-VPC shadow audits run 5 concurrent slots"])
        json.dump(bad, open(os.path.join(d, "offer", "offer.json"), "w"))
