...
- After `gtme-signals`, before `gtme-write`. Input: qualified `07-list/tam.jsonl` accounts + the ICP's `personas` + `contacts_per_account`. Output: `runs/<slug>/08-enrich/prospects.jsonl` (+ the standard folder companions `provenance.md` and `decisions.md`)
...
On hard-stop:
- Write `runs/<slug>/08-enrich/status.json`: `{"status": "blocked_no_provider", "missing_keys": [...], "at": "<iso>"}` and an **empty** `prospects.jsonl`, so the orchestrator knows why nothing advanced.
...
*(Practitioner sourcing for this section and the waterfall: `02-research/14-practitioner-signals-enrichment.md`.)*
...
The provider order below is a **starting hypothesis, not a constant.** Log every attempt to `runs/<slug>/08-enrich/attempts.jsonl`, one row per (contact × provider):
...
`gtme-write` reads `08-enrich/prospects.jsonl` (validated contacts) + their account's `08-signals/signals.jsonl` → signal-aware message per channel.
