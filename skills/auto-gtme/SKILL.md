 ...
 └──────────────────────────────────────────────────  all three confirmed together at ★1)  ─┘
          └ gtme-offer → 04-offer/offer.json ★2       (the campaign's WHAT — grand-slam gate; problems select from market-pain pain ids)
             └ gtme-sequence → 05-sequence/sequence.json ★2.5   (the SHAPE: template selected from templates/ and bound
                ...
                └ gtme-list → 06-list/tam.jsonl       (volume plan: offer_tier × goal, capped by sequence volume_ceiling)
                   ├ gtme-signals → 07-signals/signals.jsonl   ┐ (parallel — no mutual dependency)
                   └ gtme-enrich  → 07-enrich/prospects.jsonl  ┘
                      └ gtme-score → 08-score/scored.jsonl + 08-score/scored_contacts.jsonl  (waits for signals + enrich)
                         └ gtme-research → 09-research/research.jsonl   (tier-1 human_assisted accounts only)
                            └ gtme-write → 10-write/messages.jsonl ★3  (fills each bound touch per contact;
                               │                                        offer.json = the WHAT-layer menu)
                               └ gtme-send → 11-send/send_plan.jsonl ★4 (dry-run; resolves the send window per
                                  │                                      contact, enforces caps and identity gates)
                                  └ [human sends] → gtme-measure → 12-measure/measure.json ⟲
                                                     (feeds icp + score + offer + market-pain + sequence template stats)

gtme-publish → 02-publish/content_plan.jsonl   (parallel off company.json; reads offer.json opportunistically)
```

**The sequence is chosen before the list is pulled and before a word is written.** Its `volume_ceiling` bounds list size (touches x contacts against the daily cap), and its per-touch `intent` / `leans_on` / `ask` are the brief `gtme-write` fills. `gtme-send` is the operational half: real timestamps, identity gates, adapters, dry-run.
...
*(Comparative evidence: `09-research/14-practitioner-signals-enrichment.md`.)*
