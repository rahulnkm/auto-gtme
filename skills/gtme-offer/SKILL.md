...
3. **Answer the incumbent's actual complaints.** `market-pain.json tried_and_failed[].complaints[]` carries verbatim gripes about named products. An offer that solves a category-level abstraction competes on adjectives; one that answers a specific documented failure competes on evidence the buyer already believes.
4. **Design against the seller's actual motion.** Read `company.json go_to_market` first: an offer built for a `sales_led` seller with no public pricing and no docs is a different object from one built for `self_serve`. A front-end offer that assumes a buyer can try the product alone is dead on arrival when the only entry point is a demo form. Same for `stage.compliance`: a seller whose `regulatory_vocabulary` is `absent` cannot lead with a compliance claim it has never made in public.
5. **One solution per problem** — each must trace to a `company.json` capability or proof point. **Never invent a capability.** No capability → the problem stays unsolved in this offer (say so), or the offer narrows.
6. **Delivery + trim & stack** — pick delivery shapes, score value × cost, cut everything not-high-value, keep a handful of high-cost-high-value. Record the cut list in offer.md prose — the human judges what you discarded, not just what you kept.
7. **Assemble:** core offer (value-equation levers explicit) + guarantee (from the research/12 §3 menu; conditional terms = real activation points) + honest scarcity (only if it maps to a verifiable operational fact — no cap ⇒ no scarcity line) + name (MAGIC — body/asset surfaces only, never email subjects) + **front-end slice**: a **complete solution to a narrow problem whose solving reveals the problem the core offer solves** (the Problem→Solution cycle). Selected by *revelation*, never by cost — a cheap detached component is a discount, not a lead magnet. Quality bar: so good a stranger would feel obligated to pay for it standalone. Shape stays `[Type] deliverable (timeframe)`.
