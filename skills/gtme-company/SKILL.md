...
`runs/<slug>/01-company/company.json` answers exactly one question: **who is this company?** Founders (bio, online presence, relationships), what they sell and the urgent pain each feature kills, achievements, stage/funding/investors, warm network. Nothing else — no market analysis, no outreach guidance, no buyer personas (ICP's job), no value-prop list (redundant with per-feature pains), no hypothesis/evolution notes (working notes go in decisions.md).
...
**Not in this file, on purpose:** market claims of any kind (→ `02-market/market-pain.json`, including the `market_verdict` go/no-go), the thesis and founder hooks (→ `seller-research.json` — one is an interpretation, the other an unbounded pile), outreach guardrails (→ `09-write/guardrails.json`), buyer personas and signal selection (→ `03-icp/icp.json`). The schema sets `additionalProperties: false` at every level specifically so these cannot drift back in.
