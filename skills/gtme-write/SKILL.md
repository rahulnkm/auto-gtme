...
5. **CTA specific + low-cost.** "20 min before your new hire starts" beats "hop on a call". One ask.
...
7. **Follow-ups add a new argument.** Never "just bumping this". A second touch introduces a fresh angle (cost-of-delay, a new proof) or it doesn't send.
...
| `email_cold` | 1 | subject ≤50 chars, body ≤120 words | **none — links tank first-touch deliverability** | plaintext, one idea |
...
- `fully_auto` (tier-3) → templated with variable slots, still opens with the signal, flows straight to `gtme-sequence` — a human never reads it, so the anti-slop rules matter *more*, not less.
...
- Both `char_count` and `word_count` are always present. **Email channels bind on `word_count` (≤120); char-limited channels (linkedin_connect, x_*) bind on `char_count`.** Check the one that governs the channel.
...
**REFERENCE:** `research/04-psychology-nepq-persuasion.md` — NEPQ, Cialdini, cold-email copywriting for when a message needs more than the rules above.
