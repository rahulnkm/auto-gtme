...
**Every message has a brief, and it comes from `05-sequence/sequence.json`.** On a `spec: dag/1` sequence the briefs are the `nodes[]` entries whose `kind` is `message` — write one row per (contact × node id), keyed by `node_id` rather than by touch number, and cover **every** message node in the graph including the warm ones most contacts never reach. Rendering warm nodes lazily would put a generation step inside `gtme-send`'s loop, which is the single thing that architecture exists to avoid; `edges[]` is not read here, it is the engine's business. On a v1 sequence the briefs are `touches[]`, found by touch number. Either way the entry It carries `intent` (what this touch is FOR), `leans_on` (pain, objection, proof, new_angle, signal or breakup), `ask` (the size of the yes being requested), `word_max`, and `binds` (the exact `pain_id`, `objection_id` or `front_end_offer` this touch draws on). **Never pick the angle yourself** - the arc is a campaign decision made and gated at ★2.5, and a writer choosing per message turns a designed sequence back into five variations of touch 1.
...
**Answer the objection this persona actually raises.** `market-pain.json predicted_objections[]` is keyed by persona and evidenced. Look up the recipient's role, read the objection, and check `answered_by` for the offer element that handles it. The first-touch persona's strongest objection is often existential ("if this works I am automating my own job") — a message that pitches efficiency straight into it argues for the reader's redundancy. Where `answered_by` is null the objection is unhandled: do not raise it, and do not write copy that walks into it.

**Statistics come from `market-pain.json market_pain_stats[]`, never from memory.** Each entry carries `source` and `scope`; quote at most one, and only when its `scope` covers this reader's segment. A statistic scoped to US banks does not describe a marketplace, and a fraud leader will know. Check the year in the source before using it.

**Acceptance and asks.** `offer.json acceptance_components` pre-resolves the data and legal gate — attach the handling terms with the ask rather than waiting to be challenged, and offer the zero-export fallback where a CSV would need legal. Take the first rung of the selected row's `acceptance_path` as the ask; never the whole ladder. `urgency_facts` and `bonuses` reach copy under the same honesty rule as `scarcity_facts`: empty list means the message carries none.
...
Sender name comes from `runs/<slug>/channel-plan.json` `sender_identity.sender_name`, inserted as `{{sender_token}}` — except `human_assisted` runs with a `persona.md`, where the real name may be written directly (a bespoke personal email with a merge token is itself a tell). **Never invent or guess a sender name.** No calendar link unless `sender_identity.booking_link` is set. **A null in `sender_identity` blocks sending, not just rendering** — that rule was written inside channel-plan.json itself while no skill opened the file, so it bound nobody; it is enforced here now. `compliance_footer` and `physical_address_for_canspam` are null in the live run, and both are legally required on cold email.
