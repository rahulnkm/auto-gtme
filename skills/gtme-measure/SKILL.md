...
- After a cycle's `gtme-sequence` sends have outcomes. Input: send outcomes (replies, meetings, bounces) + the cycle's `08-signals/signals.jsonl` / `05-icp/icp.json` / `10-write/messages.jsonl` (`pain_id` tags) / `04-market/market-pain.json`. Output: `runs/<slug>/13-measure/measure.json` + `13-measure/measure.md`
