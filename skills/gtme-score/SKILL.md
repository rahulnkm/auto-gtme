...
**`confidence` is a gate, not a term.** It never enters `contact_score`. Confidence measures how well-sourced the *record* is, not how good the *prospect* is — mixing them makes a well-verified junior analyst outrank a shakily-sourced Head of Fraud, and produces contacts whose top-ranked reason for existing is "we are sure they exist." It is emitted as part of `send_gate` instead. The gate holds the send; the rank stays about the person. Same for `email_status` — an unvalidated address blocks the email channel, it does not demote the human.

**`send_gate` is an allowlist, and it lives in `gate.py`.** It is the one part of the formula not in `score.py`, because it needs unit tests and `score.py` runs its whole pipeline at import. `ready` requires ALL of: `record_status` exactly `verified`, `confidence` ≥ 0.7, and an `identity.pulled` date inside `icp.scoring.identity_max_age_days` (default 30). `not_found` / `wrong_person` / `stale` are `do_not_send`. Everything else — `ambiguous`, `unchecked`, an absent status, a status nobody recognises — is `verify_first`.
...
| `confidence` used as a ranking term | It gates (`send_gate`), it does not rank. It describes the record, not the prospect. |
