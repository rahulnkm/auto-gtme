...
**`confidence` is a gate, not a term.** It never enters `contact_score`. Confidence measures how well-sourced the *record* is, not how good the *prospect* is — mixing them makes a well-verified junior analyst outrank a shakily-sourced Head of Fraud, and produces contacts whose top-ranked reason for existing is "we are sure they exist." Emit it as `send_gate` instead: `ready` at ≥0.7, else `verify_first`. The gate holds the send; the rank stays about the person. Same for `email_status` — an unvalidated address blocks the email channel, it does not demote the human.
